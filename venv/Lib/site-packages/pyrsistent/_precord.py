...
class _PRecordMeta(type):
    def __new__(mcs, name, bases, dct):
        ...
        dct['_precord_mandatory_fields'] = \
            set(name for name, field in list(dct['_precord_fields'].items()) if field.mandatory)

        dct['_precord_initial_values'] = \
            dict((k, field.initial) for k, field in list(dct['_precord_fields'].items()) if field.initial is not PFIELD_NO_INITIAL)
...
class PRecord(PMap, CheckedType):
    ...
    def __repr__(self):
        return "{0}({1})".format(self.__class__.__name__,
                                 ', '.join('{0}={1}'.format(k, repr(v)) for k, v in list(self.items())))
    ...
    def serialize(self, format=None):
        ...
        """
        return dict((k, serialize(self._precord_fields[k].serializer, format, v)) for k, v in list(self.items()))
