...
class PMap(object):
    ...
    def values(self):
        return pvector(iter(self.values()))

    def keys(self):
        return pvector(iter(self.keys()))

    def items(self):
        return pvector(iter(self.items()))
    ...
    def __eq__(self, other):
        ...
        if isinstance(other, PMap):
            ...
            if self._buckets == other._buckets:
                return True
            return dict(iter(self.items())) == dict(iter(other.items()))
        elif isinstance(other, dict):
            return dict(iter(self.items())) == other
        return dict(iter(self.items())) == dict(six.iteritems(other))
    ...
    def __hash__(self):
        if not hasattr(self, '_cached_hash'):
            self._cached_hash = hash(frozenset(iter(self.items())))
