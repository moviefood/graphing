...
def _get_valid_attributes(object_name, parent_object_names):
    ...
    for attributes_dict in list(attributes.values()):
        for key, val in list(attributes_dict.items()):
            if key not in GRAPH_REFERENCE['defs']['metaKeys']:
                valid_attributes.add(key)
        deprecated_attributes = attributes_dict.get('_deprecated', {})
        for key, val in list(deprecated_attributes.items()):
            if key not in GRAPH_REFERENCE['defs']['metaKeys']:
                valid_attributes.add(key)
...
def get_deprecated_attributes(object_name, parent_object_names=()):
    ...
    for attributes_dict in list(attributes.values()):
        ...
        for key, val in list(deprecated_attributes_dict.items()):
            if key not in GRAPH_REFERENCE['defs']['metaKeys']:
...
def _get_role(object_name, attribute, value_type, parent_object_names=()):
    ...
    matches = []
    for attributes_dict in list(attributes_dicts.values()):

        for key, val in list(attributes_dict.items()):
            if key == attribute:
                matches.append(val)

        for key, val in list(attributes_dict.get('_deprecated', {}).items()):
            if key == attribute:
                matches.append(val)
...
def _get_classes():
    ...
    for class_name, class_dict in list(_BACKWARDS_COMPAT_CLASS_NAMES.items()):
...
OBJECT_NAME_TO_CLASS_NAME = {class_dict['object_name']: class_name
                             for class_name, class_dict in list(CLASSES.items())
