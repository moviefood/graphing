...
def create_choropleth(fips, values, scope=['usa'], binning_endpoints=None,
    ...
    color_lookup = dict(list(zip(LEVELS, colorscale)))
    x_traces = dict(list(zip(LEVELS, [[] for i in range(len(LEVELS))])))
    y_traces = dict(list(zip(LEVELS, [[] for i in range(len(LEVELS))])))
    ...
    for state in scope:
        if state.lower() == 'usa':
            ...
            if state in list(st_to_state_name_dict.keys()):
