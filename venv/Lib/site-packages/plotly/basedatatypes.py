...
class BaseFigure(object):
    ...
    def _repr_mimebundle_(self, include, exclude, **kwargs):
        ...
        if pio.renderers.render_on_display:
            ...
            if include:
                data = {k: v for (k, v) in list(data.items()) if k in include}
            if exclude:
                data = {k: v for (k, v) in list(data.items()) if k not in exclude}
            return data
        else:
            return None
    ...
    def data(self, new_data):
        ...
        duplicate_uids = [
            uid for uid, count in list(uid_counter.items()) if count > 1
        ]
        if duplicate_uids:
            err_msg = (
        ...
        if not all([i1 == i2 for i1, i2 in zip(new_inds, current_inds)]):
            ...
            new_inds, moving_traces_data = list(zip(
    ...
    def _perform_plotly_restyle(self, restyle_data, trace_indexes):
        ...
        # Process each key
        # ----------------
        for key_path_str, v in list(restyle_data.items()):
        ...
        if isinstance(key_path_str, string_types) and \
            ...
            for key in key_path2:
                if '_' in key[1:]:
                    ...
                    # from being split up
                    for under_prop, hyphen_prop in list(underscore_props.items()):
...
""", DeprecationWarning)
    ...
    def _perform_plotly_relayout(self, relayout_data):
        ...
        # Process each key
        # ----------------
        for key_path_str, v in list(relayout_data.items()):
...
""".format(key_path_str=key_path_str))
    ...
    def _dispatch_layout_change_callbacks(self, relayout_data):
        ...
        # ---------------------------------------
        for path_tuple, changed_paths in list(dispatch_plan.items()):
    ...
    def _build_update_params_from_batch(self):
        ...
        all_props = sorted(
            set([
                prop for trace_style in list(self._batch_trace_edits.values())
        ...
        # Fill in values
        for trace_ind, trace_style in list(batch_style_commands.items()):
            for trace_prop, trace_val in list(trace_style.items()):
    ...
    def _perform_batch_animate(self, animation_opts):
        ...
        if self._batch_trace_edits:
            animate_styles, animate_trace_indexes = list(zip(
                *[(trace_style, trace_index) for trace_index, trace_style in
                  list(self._batch_trace_edits.items())]))
        else:
...
class BasePlotlyType(object):
    ...
    def _process_kwargs(self, **kwargs):
        ...
        if invalid_kwargs and not self._skip_invalid:
            self._raise_on_invalid_property_error(*list(invalid_kwargs.keys()))
    ...
    def __repr__(self):
        ...
        props = {p: v for p, v in list(props.items())
    ...
    def _dispatch_change_callbacks(self, changed_paths):
        ...
        # ------------------------------
        for prop_path_tuples, callbacks in list(self._change_callbacks.items()):
...
class BaseLayoutType(BaseLayoutHierarchyType):
    ...
    def _process_kwargs(self, **kwargs):
        ...
        for prop, value in list(subplot_kwargs.items()):
