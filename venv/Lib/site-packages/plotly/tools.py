...
def make_subplots(rows=1, cols=1,
    ...
    col_seq = list(range(cols))[::COL_DIR]
    row_seq = list(range(rows))[::ROW_DIR]
    ...
    # title position must be calculated for each subplot
        ...
        x_dom_vals = [k for k in list(layout.to_plotly_json().keys()) if 'xaxis' in k]
        y_dom_vals = [k for k in list(layout.to_plotly_json().keys()) if 'yaxis' in k]
